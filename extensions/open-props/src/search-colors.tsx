...
export default function Command() {
  ...
  const { isLoading, data } = useCachedPromise((_) => fetchVars(colorConfig), [latestVersion], {
    execute: !!latestVersion,
  });

  return (
    <Grid isLoading={isLoading} searchBarPlaceholder="Search colors by name and shade..." columns={7}>
      {data &&
        data.map((section) => (
